...
// Movimiento en pag secundarias 


$(document).ready(function () {
  // Aplica la clase 'visible' a la primera imagen/video al cargar la página
  ...
  $(window).on('scroll', function () {
      $('.imgs_video').each(function () {
          ...
          var scrollTop = $(window).scrollTop(); // Posición del scroll actual
          var windowHeight = $(window).height(); // Altura de la ventana visible

          // Condición simple: el elemento está dentro de la ventana visible
          if (elementOffset < (scrollTop + windowHeight - 100) && elementOffset + $(this).outerHeight() > scrollTop) {
              $(this).addClass('visible');
          } else {
              $(this).removeClass('visible'); // Si quieres que desaparezcan al salir
          }
      });
  });
});
